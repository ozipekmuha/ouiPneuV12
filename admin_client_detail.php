...
try {
    ...
    $sql_client = "SELECT u.id_utilisateur, u.nom, u.prenom, u.email,
                          u.telephone, u.date_inscription, u.derniere_connexion
                   FROM Utilisateurs u
    ...
    // Ces fonctions doivent utiliser les bons noms de colonnes pour joindre/filtrer si nécessaire
...
<head>
    ...
    <style>
        ...
        .admin-action-btn.view-btn { font-size:0.8em !important; padding:0.3em 0.6em !important; text-decoration:none !important; border-radius:4px !important; border:1px solid var(--accent-primary) !important; color: var(--accent-primary) !important; background-color: var(--bg-surface) !important; }
...
<body>
        ...
        <main class="admin-content-area">
            ...
            <div class="client-details-grid">
                <div class="detail-card client-info-card">
                    ...
                    <p><strong>Nom:</strong> <?php echo sanitize_html_output(trim($client_details['prenom'] . ' ' . $client_details['nom'])); // Modifié ?></p>
                    <p><strong>Email:</strong> <?php echo sanitize_html_output($client_details['email']); // Modifié ?></p>
                    <p><strong>Téléphone:</strong> <?php echo sanitize_html_output($client_details['telephone'] ?: 'Non fourni'); // Modifié ?></p>
                    <p><strong>Date d'inscription:</strong> <?php echo sanitize_html_output(date("d/m/Y H:i", strtotime($client_details['date_inscription']))); // Modifié ?></p>
                ...
                <div class="detail-card addresses-card">
                    ...
                    <?php if (!empty($client_addresses)): ?>
                        <?php foreach($client_addresses as $address): ?>
                            <address>
                                <strong><?php echo sanitize_html_output(ucfirst($address['type_adresse'])); // Supposant type_adresse = 'livraison' ou 'facturation' ?>
                                <?php if(!empty($address['est_principale_livraison']) || !empty($address['est_principale_facturation'])) echo " (Principale)"; ?>
                                </strong><br>
                                <?php echo sanitize_html_output($address['destinataire_nom_complet'] ?? ($client_details['prenom'] . ' ' . $client_details['nom'])); // Fallback au nom du client ?><br>
                    ...
                    <tbody>
                        <?php foreach ($client_orders as $order): ?>
                            <tr>
                                ...
                                <td style="text-align:right;"><?php echo sanitize_html_output(number_format($order['montant_total_ttc'], 2, ',', ' ')) . ' €'; ?></td>
                                <td>
            ...
            <div class="client-actions">
                <a href="mailto:<?php echo sanitize_html_output($client_details['email']); ?>?subject=Concernant votre compte Ouipneu.fr" class="cta-button secondary"><i class="fas fa-envelope"></i> Contacter le Client</a>
    ...
    <script>
        ...
        const notificationClientPage = document.querySelector('.global-notification-bar');
        if (notificationClientPage) {
            setTimeout(() => {
                notificationClientPage.style.display = 'none';
            }, 5000);
        }
    </script>
</body>
